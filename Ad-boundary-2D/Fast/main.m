clc; clear;
% Paper method of Legender construction
% numerical derivative of the Legender polynomials
% Add Numann boundary condition
global idx
% profile on
p = 40;                             % degree of the polynomial
...
% Add line integral
    ...
    [~, ~, ~, ~, Bx0, Bx1, w] = Linex(p,l1,u1,l2,u2,N);
    [~, ~, ~, ~, B0y, B1y, ~] = Liney(p,l1,u1,l2,u2,N);
    
%     [ux_x0, ux_x1, uy_0y, uy_1y, dux0, dux1, du0y, du1y] = ...
%     Line_int(Xi, p, Bx_x0, Bx_x1,Bx_0y, Bx_1y, By_x0, By_x1, By_0y, By_1y);
    
    f1 = @(x,y) Mc_2D(0.5,0.5,x,y,1);
    f2 = @(x,y) Mc_2D(0.5,0.5,x,y,2);
    [fx_0y, fx_1y, fy_x0, fy_x1, gx0, gx1, g0y, g1y] = gradf(f1,f2,N,l1,u1,l2,u2);
    % h1 = f(1.5*ones(1,N),linspace(0,1,N))./sqrt(f(1.5*ones(1,N),linspace(0,1,N)).^2+1e-8);
    % h2 = f(0.5*ones(1,N),linspace(0,1,N))./sqrt(f(0.5*ones(1,N),linspace(0,1,N)).^2+1e-8);
...
%        L(i) = ((u-l)/2)*(sum(I1)+sum(I3)+sum(I2)+sum(I4));
       I1 = 0;
